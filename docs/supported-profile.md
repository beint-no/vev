...
Vev targets a safe subset of the Jakarta Persistence 4.0.0-M6 annotation model. It is not a Jakarta Persistence provider and has not passed the Jakarta Persistence TCK.
...
The native surface is implemented for generated point reads, ordered bounded batch reads, insert, versioned update/upsert/delete, append-only insert/read, and `PgQueries.scanById`. The scan orders by the generated ID, requires a `QueryLimit` from 1 through 1,000, fetches at most one sentinel row beyond that limit, materializes detached results, and reports `hasMore`. The runtime accepts only its internal scan representation; implementing `BoundedQuery` does not grant an arbitrary SQL execution path.

The Jakarta adapter currently supports detached `find`/`get` and their ordered multiple variants, cache-mode options as no-cache semantics, `fetch()` as a no-op for already-loaded supported values, and single version-qualified delete. Immutable Jakarta `void` insert, update, upsert, and refresh operations are rejected before SQL; use the typed native API that returns the new snapshot. Non-empty Jakarta batch delete is rejected before SQL because a late recoverable conflict may otherwise expose ordered partial effects; the native batch API returns every result explicitly. The following Jakarta or framework surfaces are outside the safe profile unless a later release explicitly lists them:
