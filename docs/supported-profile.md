...
| Shared reference entity | Explicit `@VevShared` plus `@VevReadOnly`; ID-only key; no tenant column; SELECT-only, intentionally visible to all model tenants | Reject implicit sharing, writes, mixed ownership, RLS policies, references to tenant-owned rows, and models with no tenant-owned mapping |
...
| Foreign keys | Only explicit scalar `@VevReference` within the closed model | Exact scoped-composite or shared-scalar keys, matching types and bounds, no cascade/deferral, and verified built-in enforcement |
