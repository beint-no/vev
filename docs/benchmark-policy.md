...
The preserved A–B–B–A report was captured from an earlier Vev commit. It covers only transaction setup, point read, ordered multi-ID reads, and an ID-bounded scan. It does not measure `@VevIndex` equality/null queries, typed-array `insertMultiple`, all-or-nothing update batches, or the revised immutable connection baseline. Those results must not be attributed to the current tranche; a new campaign and raw evidence bundle are required. Hibernate ORM `8.0.0.Beta1` also remains a prerelease baseline, not evidence about a final Hibernate ORM 8 release.
