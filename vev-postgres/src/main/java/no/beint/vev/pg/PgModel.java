...
public final class PgModel<M, T> {
    ...
    private static final Set<String> DATABASE_TYPES = Set.of(
            ...
            "date", "timestamp", "timestamptz", "bytea");
    ...
    private static void validatePlan(PgPlan<?, ?, ?, ?> plan) {
        ...
        for (PgColumn column : columns) {
            if (column.codec() == PgCodecs.BINARY && plan.checkConstraints().stream()
                    .filter(check -> check.binaryColumn().equals(column.name()) && check.maximumBytes() == column.maximumLength()).count() != 1) {
                throw new IllegalArgumentException("Binary column requires one generated byte-length check: " + column.name());
            }
        }
        for (PgCheck check : plan.checkConstraints()) {
            if (!check.binaryColumn().isEmpty() && columns.stream().noneMatch(column -> column.name().equals(check.binaryColumn())
                    && column.codec() == PgCodecs.BINARY && column.maximumLength() == check.maximumBytes())) {
                throw new IllegalArgumentException("Binary check does not match its generated column bound");
            }
        }
    ...
    private static long maximumIndexKeyBytes(PgColumn column) {
        if (column.codec() == PgCodecs.BINARY) return Math.addExact(32L, column.maximumLength());
