...
Explicit [read-only mappings](docs/read-only-mappings.md) expose no mutation capabilities and require SELECT-only table access, even inside write transactions. Stored identity/version metadata remains verifiable without granting writes. Explicit [shared reference mappings](docs/shared-reference-mappings.md) combine `@VevShared` with read-only access for rows intentionally visible to every tenant. Models containing only shared records declare `@VevModel(tenantType = ...)` and retain real tenant scopes and the same verified authority.
