...
Vev explores an ahead-of-time, PostgreSQL-first persistence model for modern JVM applications. Instead of recreating a stateful ORM session, Vev compiles a deliberately small Jakarta Persistence annotation profile into immutable entity metadata and direct bind/read plans used by a stateless persistence API. Unsupported mappings are intended to fail compilation rather than acquire approximate runtime behavior.
...
- a closed, documented subset of Jakarta Persistence rather than the complete specification.
...
The Jakarta-facing abstraction is the preview [`EntityAgent`](https://jakarta.ee/specifications/persistence/4.0/apidocs/jakarta.persistence/jakarta/persistence/entityagent), which performs operations without a persistence context and returns detached entities. Vev's selected `EntityAgent` surface maps onto the native `TransactionExecutor`, lexical `ReadTx`/`WriteTx`, and explicit `ReadEntities`/`WriteEntities` contracts. Generated per-entity plans contain binders, row readers, and mapping metadata; the PostgreSQL runtime constructs and caches the only permitted SQL shapes from that validated metadata.

The current immutable-record adapter supports detached `find`/`get`, ordered multiple lookups, and single version-qualified delete. Jakarta's `void` insert, update, upsert, and refresh operations are rejected before SQL because an immutable snapshot cannot be synchronized in place. Non-empty Jakarta batch delete is also rejected before SQL because its ordered recoverable-failure contract can expose partial effects. Use the native typed mutation APIs when database-produced state or exhaustive batch outcomes must be returned.
...
| `vev-jakarta4` | Selected Jakarta Persistence 4.0 milestone `EntityAgent` and annotation adapter |
...
Vev is not a drop-in Hibernate replacement. Source annotations are reusable only when an entity fits the accepted profile, and annotation compatibility does not imply lifecycle, transaction, query, locking, or caching compatibility. Hibernate-specific annotations are not part of the initial public profile unless a document names them explicitly.
